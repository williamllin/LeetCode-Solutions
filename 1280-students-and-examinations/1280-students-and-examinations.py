...
def students_and_examinations(students: pd.DataFrame, subjects: pd.DataFrame, examinations: pd.DataFrame) -> pd.DataFrame:
    ...
    base_df = students.merge(subjects, how = 'cross')
    exam_counts = examinations.groupby(['student_id','subject_name']).size().reset_index(name='attended_exams') #extra column: attended_exams

    result = base_df.merge(exam_counts, on=['student_id','subject_name'], how='left')
    #left join so keep all on left

    result['attended_exams'] = result['attended_exams'].fillna(0).astype(int)#deal with NA
    result = result.sort_values(by=['student_id','subject_name'])
    return result[['student_id','student_name','subject_name','attended_exams']]
